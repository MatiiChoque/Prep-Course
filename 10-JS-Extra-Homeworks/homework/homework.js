...
function numberOfCharacters(string) {
  ...
  for (let i=0; i<string.length; i++){
    if (par.hasOwnProperty(string[i])){
      par[string[i]] = par[string[i]] + 1
    }else{
      par[string[i]]=1
    }
  };
...
function asAmirror(str) {
  ...
  //Escribe tu código aquí
  let frase=str.split(' ')
    espejo = frase.map(function(elemento){
...
function deleteAbc(cadena){
  ...
  //Escribe tu código aquí
    let string = cadena.split('').filter(elemento=> elemento!=='a' && elemento!=='b' && elemento!=='c')
    return string.join('')
}
...
function sortArray(arr) {
  ...
  //Escribe tu código aquí
  let cadena=arr.sort(function(a,b){
